...
## Модуль model_inference.py
Назначение:
Модуль для предсказания ключевых HR-показателей (зарплаты и грейда) на основе новых вакансий.

### Основные функции
### predict_salary(features: dict) -> float
   Предсказывает среднюю зарплату по новой вакансии.
   Вход: словарь с признаками вакансии
   Выход: прогноз зарплаты (float, рубли)

### predict_grade(features: dict) -> int
   Автоматически определяет грейд вакансии (junior/middle/senior/lead).
   Вход: словарь с признаками вакансии
   Выход: грейд (0 — junior, 1 — middle, 2 — senior, 3 — lead) 

## Аналитические функции

### top_5_skills

`top_5_skills(title=None, area_id=None, grade=None)`

- **Назначение:** Топ-5 востребованных навыков среди вакансий по фильтру.
- **Вход:**  
    - title (str, опционально) — профессия  
    - area_id (int, опционально) — регион  
    - grade (int/str, опционально) — грейд (например, 2 или "От 3 до 6 лет")
- **Выход:** DataFrame с колонками `skill_name`, `frequency`, `mean_salary`

---

### compare_vacancy_to_market

`compare_vacancy_to_market(vac: dict)`

- **Назначение:** Сравнить свою вакансию с рынком (зарплата, навыки)
- **Вход:**  
    - title (str)  
    - area_id (int)  
    - experience_hh (str, например, "От 1 года до 3 лет")  
    - skills (list или строка)  
    - salary_rub (int/float, опционально)
- **Выход:** строка-отчёт с анализом.

**Примеры использования см. в `test_model_inference.py`**
